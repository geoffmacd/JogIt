...
- (void)setupSlidingScrollView {
    CGRect frame = self.view.bounds;
    _sliderOpeningWidth =  frame.size.width + kDefaultVisiblePortion;
...
-(UIPanGestureRecognizer*)getSliderPanGesture
{
    ...
    return _slidingScrollView.panGestureRecognizer;
...
- (void)updateInterface {
    ...
    //set to be x= 0 if the menu is being displayed @ open
    if (self.isOpen) {
...
- (void)setFrontViewController:(UIViewController *)viewController animated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    CGRect frame = self.view.bounds;
    newFrontViewController.view.frame = CGRectMake(0, frame.origin.y, frame.size.width, frame.size.height);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //do not drag logger if in logger view
    if(!isSetupForPauseScroll)
    {
        //move back controller with scroll
        CGRect backRect = self.backViewController.view.frame;
        //keep logger exactly at left of screen
        backRect.origin.x = co.x;
        [self.backViewController.view setFrame:backRect];
        
        //shade drop shadow
        [self.frontViewControllerDropShadow_right setAlpha:1 - (co.x / _sliderOpeningWidth)];
        
        //shade logger view when initially opening
        //[self.backViewController.view setAlpha:0.5+(co.x / (2*_sliderOpeningWidth))];
    }
    ...
    {
        //remove pause image, put logger back to background @ x=0
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_animating == NO) {
        ...
        if ( (origin.x >= 0) && (scrollView.dragging == NO) ){
            CGRect rect = _slidingScrollView.frame;
            rect.origin.x = 0;
            _slidingScrollView.frame = rect;
            _slidingScrollView.contentOffset = CGPointMake(0, 0);
