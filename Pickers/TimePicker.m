...
+ (id)showPickerWithTitle:(NSString *)title rows:(NSArray *)strings initialSelection:(NSInteger)index doneBlock:(TimeActionStringDoneBlock)doneBlock cancelBlock:(TimeActionStringCancelBlock)cancelBlockOrNil origin:(id)origin {
    TimePicker * picker = [[TimePicker alloc] initWithTitle:title rows:strings initialSelection:index doneBlock:doneBlock cancelBlock:cancelBlockOrNil origin:origin];
    [picker showRunFormPicker];
    return picker ;
}

- (id)initWithTitle:(NSString *)title rows:(NSArray *)strings initialSelection:(NSInteger)index doneBlock:(TimeActionStringDoneBlock)doneBlock cancelBlock:(TimeActionStringCancelBlock)cancelBlockOrNil origin:(id)origin {
...
+ (id)showPickerWithTitle:(NSString *)title rows:(NSArray *)data initialSelection:(NSInteger)index target:(id)target successAction:(SEL)successAction cancelAction:(SEL)cancelActionOrNil origin:(id)origin {
    TimePicker *picker = [[TimePicker alloc] initWithTitle:title rows:data initialSelection:index target:target successAction:successAction cancelAction:cancelActionOrNil origin:origin];
    [picker showRunFormPicker];
    return picker;
}
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if(component == 0)
        return [NSString stringWithFormat:@"%d h" , row];
